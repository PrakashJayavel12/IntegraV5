          ...
          for (int index1 = 0; index1 < dataSet1.Tables[1].Rows.Count; ++index1)
          {
            if (!(dataSet1.Tables[1].Rows[index1]["Selected"].ToString() == string.Empty) && !(dataSet1.Tables[1].Rows[index1]["Selected"].ToString() == "False"))
            {
              ...
              //  this.GetDatesBetween(this.StartDate, this.EndDate);
                ...
                string dfrom1 = Convert.ToDateTime(StartDate).ToString("yyyy-MM-dd");
                string dto1 = Convert.ToDateTime(EndDate).ToString("yyyy-MM-dd");

                string Fdate = dfrom1.ToString().Replace("/", "-");
                string Tdate = dto1.ToString().Replace("/", "-");
                ///////////////////https://login.microsoftonline.com/d356af53-f35f-4eff-86e5-5c676313e4e4/oauth2/token


                //string line = "";

                //ServicePointManager.SecurityProtocol = (SecurityProtocolType)768 | (SecurityProtocolType)3072;
                //String s1 = server2 + "?branchId=" + center;
                ...
                //{
                //    ///var str = "{\"fromDate\":\"" + dfrom + "\",\"toDate\":\"" + dto + "\"}";
                //    var str = "{\"auth_token\":\"" +Appkey  + "\",\"form_date\":\"" + dfrom + "\",\"to_date\":\"" + dto + "\"}";
                ...
                //XmlDocument xmlDoc = (XmlDocument)JsonConvert.DeserializeXmlNode(line, "Trans");
                //string xmlString = xmlDoc.DocumentElement.InnerXml;
                //dtsMaster.ReadXml(new XmlNodeReader(xmlDoc));
                ...
                DateTime d2 = EndDate;
                DateTime d3 = StartDate.AddDays(1);

                DateTime noww = DateTime.Now;


                string dayOfMonth = noww.Day.ToString();                      // Day of the month (e.g., "30")
                string month = noww.Month.ToString();                         // Month (e.g., "10")
                string monthLetter = noww.ToString("MMMM")[0].ToString();   // First letter of the month (e.g., "O")
                string dayLetter = noww.ToString("dddd")[0].ToString();     // First letter of the day of the week (e.g., "W")
                string amPm = noww.ToString("tt", CultureInfo.InvariantCulture); // Time format (AM/PM)
                string am = amPm.Substring(0, 1);

                string key1 = dayOfMonth + month + monthLetter + dayLetter + am;

                //$"{dayOfMonth}{month}{monthLetter}{dayLetter}{am}";

                Writelog(key1.ToString(), "Key");

                try
                {

                    string L_URL = "https://ideacrate.odoo.com/web/session/authenticate";
                    CookieContainer cookieContainer = new CookieContainer();
                    HttpWebRequest httpWebRequest1 = (HttpWebRequest)HttpWebRequest.Create(L_URL);
...
@"    ""params"": {
                    " + "\n" +
@"        ""db"":""orange-uae-production-10490588"",
                    " + "\n" +
@"        ""login"": ""Path.Finderaha"",
                    " + "\n" +
@"        ""password"": ""Path.Finderaha""
                    " + "\n" +
@"    }
                    " + "\n" +
@"}
...
@"";
                    ...
                    foreach (Cookie cookie in httpResponse1.Cookies)
                    {
                    ...
                    {
                        string line1 = streamReader1.ReadToEnd();
                    ...
                    do
                    {
                       ...
                       String dfrom, dto;
                        ...
                        d3 = d1.AddDays(1);
                        dfrom = Convert.ToDateTime(d1).ToString("yyyy-MM-dd");
                        dto = Convert.ToDateTime(d3).ToString("yyyy-MM-dd");

                        string s1 = AccountName;

                        string state = "done";
                        HttpWebRequest httpWebRequest = (HttpWebRequest)HttpWebRequest.Create(s1);
                        httpWebRequest.Method = "POST";
                        httpWebRequest.ContentType = "application/json";
                        //ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls11 | SecurityProtocolType.Tls12;

                       // httpWebRequest.CookieContainer = new CookieContainer();
                        httpWebRequest.Headers.Add("Cookie", "session_id=" + cookievalue);

                        // httpWebRequest.Headers.Add("Cookie", "session_id="6902ef0adf5bc5a1946c60728a9282efc074e1b1);
                ...
                {
                    TextWriter twEr = new StreamWriter(Application.StartupPath + "\\SyncError\\SyncError" + DateTime.Now.ToString("HHmmss") + ".txt");
                    twEr.WriteLine(ee.StackTrace.ToString());
                    twEr.Close();
                ...
                if (dtsMaster.Tables.Count <= 0 || dtsMaster == null)
                  return;
                dtsMaster.Tables["sales_orders"].TableName = "data";
                ...
                if (dtsMaster.Tables["data"].Rows.Count > 0)
                {
                  ...
                  {
                      if (!string.IsNullOrEmpty(row["date_order"].ToString()))
                    {
                        string str22 = row["date_order"].ToString();
                        string date1 = str22.Substring(0, 10);
                        row["Cdate"] = DateTime.ParseExact(date1, "yyyy-MM-dd", CultureInfo.InvariantCulture).ToString("dd/MMM/yyyy");                     
                      row["Cdate1"] = (object) Convert.ToDateTime(row["Cdate"]).ToString("yyyyMMdd");
                      this.Writelog(row["Cdate"].ToString(), "Data");
                      this.Writelog(row["Cdate1"].ToString(), "Data");
                    }
                  }
 ...
 //                   string billtime = "";
 //                   decimal taxvalue = 0;
 //                   decimal Tendervalue = 0;
 //                   //decimal Netvalue = "";
 //                   //decimal taxvalue = "";
 //                   //decimal Retvalue = ""; 
 //                   //string discountvalue = "";

 //                   string ReceiptNo = dr["RCPT_NUM"].ToString();
 //                   //string taxvalue = dr["TAX_AMT"].ToString();
 //                   string dte = dr["cdate"].ToString();
 //                   Transaction = dr["TRAN_STATUS"].ToString();
 //                   string pay = "";
 //                   DataRow[] drTrans;
 //                   drTrans = dtsMaster.Tables["data"].Select("RCPT_NUM='" + ReceiptNo.ToString() + "' ", "");

 //                   foreach (DataRow drnet in drTrans)
 //                   {

 //                       if (drnet["INV_AMT"].ToString() != string.Empty || drnet["INV_AMT"].ToString() != "")
 //                       {
 //                           Netvalue += Math.Abs(Convert.ToDecimal(drnet["INV_AMT"]));
 //                       }
 //                       if (drnet["TAX_AMT"].ToString() != string.Empty || drnet["TAX_AMT"].ToString() != "")
 //                       {
 //                           taxvalue += Math.Abs(Convert.ToDecimal(drnet["TAX_AMT"]));

 //                       }
 //                       if (drnet["RET_AMT"].ToString() != string.Empty || drnet["RET_AMT"].ToString() != "")
 //                       {
 //                           Retvalue += Math.Abs(Convert.ToDecimal(drnet["RET_AMT"]));
 //                       }
 //                       if (!string.IsNullOrEmpty(drnet["RCPT_TM"].ToString()))
 //                       {
 //                           billtime = drnet["RCPT_TM"].ToString();
 //                       }

 //                       //if (drnet["TENDER_AMOUNT"].ToString() != string.Empty || drnet["TENDER_AMOUNT"].ToString() != "")
 //                       //{
 //                       //    Tendervalue += Math.Abs(Convert.ToDecimal(drnet["TENDER_AMOUNT"]));
 //                       //}

 //                       //if (!String.IsNullOrEmpty(drnet["TENDER_AMOUNT"].ToString()) && !drnet["TENDER_AMOUNT"].ToString().Contains("null"))
 //                       //{
 //                       //    Tendervalue += Convert.ToDecimal(drnet["TENDER_AMOUNT"]);
 //                       //}

 //                       //if (drnet["RCPT_TM"].ToString() != string.Empty || drnet["RCPT_TM"].ToString() != "")
 //                       //{
 //                       //    billtime = Math.Abs(Convert.ToDecimal(drnet["RCPT_TM"]));
 //                       //}

 //                       //if (!string.IsNullOrEmpty(drs["INV_AMT"].ToString()))
 //                       //{
 //                       //    Netvalue += Math.Abs(Convert.ToDecimal(drs["INV_AMT"]));
 //                       //}

 //                       ////if (!string.IsNullOrEmpty(drs["TAX_AMT"].ToString()))
 //                       ////{
 //                       ////    taxvalue += Math.Abs(Convert.ToDecimal(drs["TAX_AMT"]));
 //                       ////}
 //                       //if (!string.IsNullOrEmpty(drs["TAX_AMT"].ToString()))
 //                       //{
 //                       //    taxvalue = drs["TAX_AMT"].ToString();
 //                       //}

 //                       //if (!string.IsNullOrEmpty(drs["RET_AMT"].ToString()))
 //                       //{
 //                       //    Retvalue += Math.Abs(Convert.ToDecimal(drs["RET_AMT"]));
 //                       //}

 //                       //if (!string.IsNullOrEmpty(drs["DISCOUNT"].ToString()))
 //                       //{
 //                       //    discountvalue += Math.Abs(Convert.ToDecimal(drs["DISCOUNT"]));
 //                       //}
 //                       //if (!string.IsNullOrEmpty(drs["RCPT_TM"].ToString()))
 //                       //{
 //                       //    billtime = drs["RCPT_TM"].ToString();
 //                       //}
 ...
 //                   string stax = Convert.ToDecimal(taxvalue).ToString("0.00");
 //                   string Tender = Convert.ToDecimal(Netvalue).ToString("0.00");




 //                   if (Transaction.ToString().Contains("R"))
 //                   {
 ...
 //                       dtot += Convert.ToDecimal(Netvalue);
 //                   }
 //                   //snet = Convert.ToDecimal(Math.Abs(Netvalue)).ToString("0.00");              
 //                   //string stax = Convert.ToDecimal(Math.Abs(taxvalue)).ToString("0.00");
 //                   ////    snet = Convert.ToDecimal(Netvalue).ToString("0.00");
 //                   ////    Transaction = "SALES";                    
 //                   ////    dtot += Convert.ToDecimal(Netvalue);               
 //                   //string sdis = Convert.ToDecimal(Math.Abs(discountvalue)).ToString("0.00");
 //                   //string date = Strings.Left(dr["RCPT_DT"].ToString(), 10).ToString().Replace("-", "");
 //                   //string time = Strings.Right(dr["RCPT_DT"].ToString(), 8).ToString().Replace(":", "");
 //                   //billtime = dr["RCPT_TM"].ToString();
 //                   billtime = billtime.Replace(":","");
 //                   string billno = dr["RCPT_NUM"].ToString();
 ...
 //    billno,                 // RCPT_NUM
 //    billdate,               // RCPT_DT
 //    bussdate,               // BUSINESS_DT
 //    billtime,               // RCPT_TM
 //    snet.ToString(),        // INV_AMT
 //    stax.ToString(),        // TAX_AMT
 //    sret.ToString(),        // RET_AMT
 //    "",                     // CUST_NAME
 //    "",                     // CUST_GEN
 //    "",                     // CUST_NATION
 //    " ",                    // CUST_EMAIL
 //    "",                     // CUST_CONT
 //    paxtype,                // CUST_PAX_TYPE
 //    "",                     // CUST_PP_NO
 //    custBP.ToString(),      // CUST_BP
 //    "",                     // CUST_FL_NO
 //    "",                     // CUST_FL_CLASS
 //    "",                     // CUST_BR_GATE_NO
 //    "",                     // CUST_PNR_NO
 //    "",                     // CustomerMobileNo  ← added missing
 //    "",                     // CustomerID        ← added missing
 //    "",                     // OfferCode         ← added missing
 //    "",                     // GiftVoucherNo     ← added missing
 //    Transaction.ToString(), // TRAN_STATUS
 //    "INR",                  // OP_CUR
 //    "1.000",                // BC_EXCH
 //    discountvalue.ToString(),// DISCOUNT
 //    "",                     // CUST_ORIGIN
 //    "",                     // CUST_DESTINATION
 //    " ",                    // VOUCH_CODE
 //    dod.ToString()          // DATE_OF_DEPARTURE (make sure dod is convertible to string)
 //);
 
                        
                        
 //                       // CommonClass.dtPay.Rows.Add("G115", billno, billdate, "CASH", "INR", "1.000", dr["TENDER_AMOUNT"].ToString(), "INR", "1.000", dr["PAYMENT_STATUS"].ToString());

 //                       CommonClass.dtPay.Rows.Add("G115", billno, billdate,
 //                                   "CASH", "INR", "1.000",
 //                                   Tender.ToString(), "INR", "1.000",
 //                                   Transaction.ToString());

 //                       //ITEM 

 //                       DataRow[] drItem;
 //                       drItem = dtsMaster.Tables["ItemDetail"].Select("RCPT_NUM='" + ReceiptNo.ToString() + "' ", "");
 //                       foreach (DataRow dritem in drItem)
 //                       {

 //                           CommonClass.dtItem.Rows.Add("G111", billno, billdate, dritem["ITEM_CODE"].ToString(), dritem["ITEM_NAME"].ToString(),
 //                               dritem["ITEM_QTY"].ToString(), dritem["ITEM_PRICE"].ToString(), dritem["ITEM_CAT"].ToString(), dritem["ITEM_TAX"].ToString(),
 //                           dritem["ITEM_TAX_TYPE"].ToString(), dritem["ITEM_NET_AMT"].ToString(), dritem["OP_CUR"].ToString(), dritem["BC_EXCH"].ToString(),
 //                           dritem["ITEM_STATUS"].ToString(), dritem["ITEM_DISCOUNT"].ToString());
 //                       }
 //                   }
 //                   //Payment

 //                   //DataRow[] drPay;
 //                   //drPay = dtsMaster.Tables[0].Select("RCPT_NUM='" + ReceiptNo.ToString() + "' and  cdate='" + dte.ToString() + "' and PAYMENT_STATUS='" + dr["PAYMENT_STATUS"].ToString() + "' ", "");

 //                   //foreach (DataRow drs1 in drPay)
 //                   //{
 //                   //    if (drs1["PAYMENT_NAME"].ToString().Contains("Cash"))
 //                   //    {
 //                   //        pay = "CASH";
 //                   //    }
 //                   //    else
 ...
 //                   //    }
 //                   //}
 //                   //if (Transaction.ToString() == "SALES")
 //                   //{

 //                   //    if (CommonClass.dtTrans.Select("RCPT_NUM='" + billno.ToString() + "'  and RCPT_DT='" + billdate.ToString() + "'  ", "").Length == 0)
 //                   //    {
 //                   //        CommonClass.dtPay.Rows.Add("G115", billno, billdate, pay.ToString(), "INR", "1.000", snet.ToString(), "INR", "1.000", Transaction.ToString());
 //                   //    }
 //                   //}
 //                   //else
 //                   //{
 ...
 //                   //        CommonClass.dtPay.Rows.Add("G115", billno, billdate, pay.ToString(), "INR", "1.000", sret.ToString(), "INR", "1.000", Transaction.ToString());
 //                   //    }
 //                   //}
 //               }
                ...
                string Receiptno = dr["pos_reference"].ToString();
                string ReceiptDate = dr["date_order"].ToString();
                ...
                drsNetamt = dtsMaster.Tables["data"].Select("pos_reference='" + Receiptno.ToString() + "'  and date_order='" + ReceiptDate.ToString() + "'  ", "");

                foreach (DataRow drnet in drsNetamt)
                {
                    if (drnet["amount_total"].ToString() != string.Empty || drnet["amount_total"].ToString() != null)
                    {
                        Invvalue = Convert.ToDecimal(drnet["amount_total"]);
                    }

                    if (drnet["amount_return"].ToString() != string.Empty || drnet["amount_return"].ToString() != null)
                    {
                        retvalue = Convert.ToDecimal(drnet["amount_return"]);
                    }
                    if (drnet["amount_tax"].ToString() != string.Empty || drnet["amount_tax"].ToString() != null)
                    {
                        taxvalue = Convert.ToDecimal(drnet["amount_tax"]);
                    }

                }


                billno = dr["pos_reference"].ToString();
                sret = Convert.ToDecimal(Invvalue).ToString("0.00");
                if (sret.Contains("-"))
                {
                    sret = Convert.ToDecimal(Math.Abs(Invvalue)).ToString("0.00");
                    sinv = "0.00";
                    snet = "0.00";
                    Transaction = "RETURN";
                    dtot -= Convert.ToDecimal(retvalue);
                ...
                {
                    snet = Convert.ToDecimal(Invvalue).ToString("0.00");
                    Transaction = "SALES";
                    sret = "0.00";
                    sinv = Convert.ToDecimal(Invvalue).ToString("0.00");
                    dtot += Convert.ToDecimal(Invvalue);

                }
                stax = Convert.ToDecimal(Math.Abs(taxvalue)).ToString("0.00");
                string dte = dr["date_order"].ToString();

                DateTime date = DateTime.ParseExact(dte, "yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture);

                string billdate = date.ToString("yyyyMMdd", CultureInfo.InvariantCulture);

                string billtime = date.ToString("HHmmss", CultureInfo.InvariantCulture);



               
                    //CommonClass.dtTrans.Rows.Add("G100", " ", "1", "1", billno, billdate, billdate, billtime, snet.ToString(), stax.ToString(), sret.ToString(), "", "", "", " ", "", paxtype, "", custBP.ToString(), "", "", "", "", Transaction.ToString(), "INR", "1.000", "0.00", "", "", " ", dod);
                    if (CommonClass.dtTrans.Select("RCPT_NUM='" + billno.ToString() + "'  and RCPT_DT='" + billdate.ToString() + "'  ", "").Length == 0)
                    {
                        //   CommonClass.dtTrans.Rows.Add("G100", " ", "1", "1", billno, billdate, bussdate, billtime, snet.ToString(), stax.ToString(), sret.ToString(), "", "", "", " ", "", paxtype, "", custBP.ToString(), "", "", "", "", Transaction.ToString(), "INR", "1.000", discountvalue.ToString(), "", "", " ", dod);
                        ...
                        if (Transaction.ToString() == "SALES")
                        {
                            CommonClass.dtPay.Rows.Add("G115", billno, billdate, "OTHERS", "INR", "1.000", snet.ToString(), "INR", "1.000", Transaction.ToString());
                        }
                        else
                        {
                            CommonClass.dtPay.Rows.Add("G115", billno, billdate, "OTHERS", "INR", "1.000", sret.ToString(), "INR", "1.000", Transaction.ToString());
                        }
                    }
            ...
            catch (Exception xe)
            {
                ...
                Writelog(xe.ToString(), "WritedataError");
            }
        }
