        ...
        if (dataSet1.Tables[1].Rows.Count > 0)
        {
          for (int index1 = 0; index1 < dataSet1.Tables[1].Rows.Count; ++index1)
          {
            if (!(dataSet1.Tables[1].Rows[index1]["Selected"].ToString() == string.Empty) && !(dataSet1.Tables[1].Rows[index1]["Selected"].ToString() == "False"))
            {
              ...
              server = encryption.decrypt(dataSet1.Tables[1].Rows[index1]["Server"].ToString());
                            center = dataSet1.Tables[1].Rows[index1]["CenterCode"].ToString();
              ...
              //  this.GetDatesBetween(this.StartDate, this.EndDate);
                ...
                String s1 = AccountName;// +"?branchId=" + center;
              ...
              //  line = File.ReadAllText(@"D:\PRAKASHJ\ALL_DLL_ORIGINAL\INTEGRA_V5\_DLL_ORIGINAL\API\Be_Young_Web_V5\data.txt");
                ...
                if (dtsMaster.Tables["data"].Rows.Count > 0)
                {
                  ...
                  foreach (DataRow row in (InternalDataCollectionBase) dtsMaster.Tables["data"].Rows)
                  {
                    ...
                    {
                        string str22 = row["RCPT_DT"].ToString();
                      row["Cdate"] = DateTime.ParseExact(str22, "yyyy-MM-dd", CultureInfo.InvariantCulture).ToString("dd/MMM/yyyy");                     
    ...
    public void Generate_EODFile()
    {
     ...
     dod.ToString()          // DATE_OF_DEPARTURE (make sure dod is convertible to string)
                        ...
                        foreach (DataRow dritem in drItem)
                        {
                            ...
                            Transaction, dritem["ITEM_DISCOUNT"].ToString());
                        }
                    }
                    //Payment
