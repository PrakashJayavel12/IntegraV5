...
{ 
  ...
  { 
          ...
          for (int index1 = 0; index1 < dataSet1.Tables[1].Rows.Count; ++index1)
          {
            if (!(dataSet1.Tables[1].Rows[index1]["Selected"].ToString() == string.Empty) && !(dataSet1.Tables[1].Rows[index1]["Selected"].ToString() == "False"))
            {
              ...
              //  this.GetDatesBetween(this.StartDate, this.EndDate);
                ...
                DataSet dtsTran = new DataSet();
                DataSet dtsMaster = new DataSet();
                
                //https://er4uenterpriseplus.in/er4u/giftiny/api_sale_billwise_detail3.php?auth_key=a4025192eed9582243c50a98d48ff84d&date_start=2025-05-20&date_end=2025-05-27
                ...
                ////TOKEN
                //String s1 = "https://api.forkpos.com/public/v2/Oauth/Token";  //AccountName + "?auth_key=" + Appkey + "&date_start=" + dfrom + "&date_end=" + dto;
                ...
                //{
                //    var str = "{\"clientId\":\"" + suser + "\",\"clientSecret\":\"" + spwd + "\"}";
                ...
                //string Token = JObject.Parse(line)["token"].ToString();
                //DATAA
                ...
                string Fdate = dfrom.ToString().Replace("/", "-");
                string Tdate = dto.ToString().Replace("/", "-");
                ...
                do
                {
                    string pno = pgno.ToString();
                    try
                    {
                        string s1 = AccountName + "?branch_id=" + center[0] + "&from_date=" + StartDate.ToString("yyyy-MM-dd") + "&to_date=" + EndDate.ToString("yyyy-MM-dd") + "&page=" + pno;
                        Writelog(s1, "URL");
                        ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls | SecurityProtocolType.Tls11 | SecurityProtocolType.Tls12;
                        ServicePointManager.ServerCertificateValidationCallback = delegate { return true; };                         
                        HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(s1);
                        request.Method = "GET";
                        request.ContentLength = 0;
                        request.ContentType = "application/json";
                        request.Headers.Add("Authorization", "Bearer " + Appkey);
                        //'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..wx6ynP0duy_3pt1GJ09Rct6_jO_pVrfi0qeMX-VAe9Q'
                        HttpWebResponse httpResponse = (HttpWebResponse)request.GetResponse();

                        using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                        {
                            line = streamReader.ReadToEnd();
                            Writelog(line, "LineData");
                        } 

                        string parentNote = "Trans";
                        XmlDocument xmlDoc = JsonConvert.DeserializeXmlNode("{\"" + parentNote + "\":" + line + "}", "DocumentElement");

                        dtsMaster.ReadXml(new XmlNodeReader(xmlDoc));
                        if (dtsMaster.Tables[0].Rows.Count == 0)
                        {
                            hasData = false; // No more data to fetch
                        }
                        else
                        {
                           // dtsTran.Tables.Add(dt);
                            pgno++;
                        }
                    }
                    catch (Exception https)
                    {
                        //TextWriter twEr = new StreamWriter(Application.StartupPath.ToString() + "\\Syncerror\\" + DateTime.Now.ToString("HHmmss") + ".txt", true);
                        //twEr.WriteLine(sline);
                        //twEr.WriteLine(https.Message.ToString());
                        //twEr.Close();
                        hasData = false; // Exit loop on error
                    }

                } while (hasData);

                foreach (DataTable table in dtsTran.Tables)
                {
                    dtsMaster.Tables[0].Merge(table);
                }


                if (dtsMaster.Tables.Count <= 0 || dtsMaster == null)
                  return;
                //dtsMaster.Tables["data"].TableName = "dataNo";
                dtsMaster.Tables["Transactions"].TableName = "data";
                ...
                if (dtsMaster.Tables["data"].Rows.Count > 0)
                {
                 ...
                 // dtsMaster.Tables["data"].Columns.Add("RECPT_TM");
                  foreach (DataRow row in (InternalDataCollectionBase) dtsMaster.Tables["data"].Rows)
                  {

                      if (!string.IsNullOrEmpty(row["RCPT_DT"].ToString()))
                    {
                      //  string[] DateArray;
                      //if(row["paidTime"].ToString().Contains('.'))
                      //{
                      //     DateArray = row["paidTime"].ToString().Split('.');
                      //}
                      //else
                      //{
                      //    DateArray = row["paidTime"].ToString().Split('Z');
                      //}
                        row["Cdate"] = DateTime.ParseExact(row["RCPT_DT"].ToString(), "yyyyMMdd", CultureInfo.InvariantCulture).ToString("dd/MMM/yyyy");                     
                      row["Cdate1"] = (object) Convert.ToDateTime(row["Cdate"]).ToString("yyyyMMdd");
                     // row["RECPT_TM"] = DateTime.ParseExact(str22, "yyyy-MM-ddTHH:mm:ss", CultureInfo.InvariantCulture).ToString("HHmmss");  
    ...
    public void Generate_EODFile()
    {
        ...
        private static void AddToDataTable(DataTable table, JObject obj)
        {
        ...
        private void WriteData(TextWriter tw, DataRow dr, DataSet dtsMaster)
        {
            try
            {
                ...
                { 
                  ...
                  //  string Tender = Convert.ToDecimal(Tendervalue).ToString("0.00");




                    if (!Transaction.ToString().Contains("SALES"))
                    {
                    ...
                    else 
                    {
                        ...
                        Transaction = "SALES";
                        sret = "0.00";
                        sinv = Convert.ToDecimal(Tendervalue).ToString("0.00");
                        dtot += Convert.ToDecimal(Netvalue);
                    }
                    ...
                    string billdate = dr["Cdate1"].ToString();

                    if (billtime.Length.ToString() == "5")
                    {
                        billtime = "0" + billtime;
                    }
                    ...
                    //} 
                    var result = (from DataRow row in dtsMaster.Tables["PaymentDetail"].AsEnumerable()
                                  where row.Field<string>("rcpt_num") == billno
                                  select row.Field<string>("payment_name")).FirstOrDefault();
                    string paymentMethod = CategorizePaymentMethod(result.ToString());
                    string bussdate = billdate;// Convert.ToDateTime(dr["cdate"]).ToString("yyyyMMdd");

                    if (CommonClass.dtTrans.Select("RCPT_NUM='" + billno.ToString() + "'  and RCPT_DT='" + billdate.ToString() + "'  ", "").Length == 0)
                    {
                   // CommonClass.dtTrans.Rows.Add("G100", " ", "1", "1", billno, billdate, bussdate, billtime, snet.ToString(), stax.ToString(), sret.ToString(), "", "", "", " ", "", paxtype, "", custBP.ToString(), "", "", "", "", Transaction.ToString(), "SR", "1.000", discountvalue.ToString(), "", "", " ", dod);
   ...
   " ",                     // 2  - LOCATION_CODE
   "1",                     // 3  - TERMINAL_ID
   paymentMethod,                     // 4  - SHIFT_NO
   billno,                  // 5  - RCPT_NUM
   billdate,                // 6  - RCPT_DT
   bussdate,                // 7  - BUSINESS_DT
   billtime,                // 8  - RCPT_TM
   snet.ToString(),         // 9  - INV_AMT
   stax.ToString(),         // 10 - TAX_AMT
   sret.ToString(),         // 11 - RET_AMT
   "",                      // 12 - CUST_NAME
   "",                      // 13 - CUST_GEN
   "",                      // 14 - CUST_NATION
   " ",                     // 15 - CUST_EMAIL
   "",                      // 16 - CUST_CONT
   paxtype,                 // 17 - CUST_PAX_TYPE
   "",                      // 18 - CUST_PP_NO
   custBP.ToString(),       // 19 - CUST_BP
   "",                      // 20 - CUST_FL_NO
   "",                      // 21 - CUST_FL_CLASS
   "",                      // 22 - CUST_BR_GATE_NO
   "",                      // 23 - CUST_PNR_NO
   ...
   "",                      // 26 - OfferCode
   "",                      // 27 - GiftVoucherNo
   Transaction.ToString(),  // 28 - TRAN_STATUS
   "INR",                   // 29 - OP_CUR
   "1.000",                 // 30 - BC_EXCH
   discountvalue.ToString(),// 31 - DISCOUNT
   "",                      // 32 - CUST_ORIGIN
   "",                      // 33 - CUST_DESTINATION
   " ",                     // 34 - VOUCH_CODE
   dod.ToString()           // 35 - DATE_OF_DEPARTURE
);
                        CommonClass.dtPay.Rows.Add("G115", billno, billdate,
                                    "CASH", "INR", "1.000",
                                    snet.ToString(), "INR", "1.000",
                        ...
                        {
                            DataRow[] drItem;
                            drItem = dtsMaster.Tables["ItemDetail"].Select("rcpt_num='" + billno.ToString() + "' ", "");
                            foreach (DataRow dritem in drItem)
                            {
                                string qty = dritem["item_qty"].ToString();
                                    if(qty == "" || qty == "null")
                                    {
                                        qty = "1";
                                    }
                                CommonClass.dtItem.Rows.Add("G111", billno, billdate, dritem["item_code"].ToString(), dritem["item_name"].ToString(),
                                    qty, dritem["item_price"].ToString(), dritem["item_cat"].ToString(), dritem["item_tax"].ToString(),
                                "I", dritem["item_net_amt"].ToString(), "INR", "1.000",
                                dritem["item_status"].ToString(), "0");  //dritem["ITEM_DISCOUNT"].ToString() dritem["BC_EXCH"].ToString() dritem["ITEM_CAT"].ToString()
                            }
                        }
      ...
      foreach (DataRow row in (InternalDataCollectionBase) this.datas.Tables["Transaction"].Rows)
      {
        ...
        Transactions transactions = new Transactions()
        {
          ...
          //DATE_OF_DEPARTURE = Convert.ToInt32(row["DATE_OF_DEPARTURE"].ToString()),
            ...
            GiftVoucherNo = row["GiftVoucherNo"].ToString(),
 ...
 //   private string AppPath(string AppPath) => Directory.GetParent(AppPath + "\\").Parent.FullName;
  ...
  //  public static DateTime UnixTimeStampToDateTime(long unixTimeStamp) => new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc).AddSeconds((double) unixTimeStamp).ToLocalTime();
    ...
    string CategorizePaymentMethod(string paymentMethod)
    {
        if (paymentMethod == "SWIGGY" || paymentMethod == "ZOMATO")
        {
            return "Online";
        }
        ...
        else if (paymentMethod.Contains("Razor") || paymentMethod.Contains("ex"))
        {
            return "Others";
        }
        else
